...
Two arduino_com_xxx.hpp / cpp files are available according if you use a windows or linux operating system. You can delete the one you are not compatible with and comment / uncomment the corresponding includes in `stk-code\src\karts\controller\skidding_ai.cpp` and `stk-code\src\karts\kart.cpp`.
