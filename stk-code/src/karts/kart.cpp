...
#endif
{
    ...
    m_last_race_position = m_initial_position;
    m_lap = -1;
    m_prev_lap = m_lap;
    ...
    //////////////////
    m_turn_sounds[0] = SFXManager::get()->createSoundSource("turn1");
    m_turn_sounds[1] = SFXManager::get()->createSoundSource("turn2");
    m_turn_sounds[2] = SFXManager::get()->createSoundSource("turn3");
    m_turn_sounds[3] = SFXManager::get()->createSoundSource("turn4");
...
void Kart::updatePositionAdIfDifferent(int ticks) {
    /* static int counter5s = 500;
    counter5s -= ticks; */

    if (m_last_race_position == m_race_position) return;

    //if (counter5s <= 0) {
        announceRank(m_last_race_position > m_race_position);

        // reset values and counter after announcement    
        m_last_race_position = m_race_position;
     //   counter5s = 500;
    //} 
}
...
float Kart::computeRelativeAngle(float a, float b) {
    ...
    if ((extract_sign(a) == extract_sign(b)) /* | (abs(a) < 0.00001) | (abs(b) < 0.00001) */ | (abs(diff) <= 180)) {
...
void Kart::categorizeTurns(int* angle_category, float *angle) {
    ...
    for (int i = 0; i < max_nodes; ++i) {
        ...
        next_angle = angle[next_idx];

////// IF NOT IN A TURN YET /////////////
        if (current_state == NO_TURN) {
            if (current_angle_cat < next_angle_cat) {
                // start new turn at next sector
                turn.sector_start = next_idx;
                current_state = IN_TURN;
            } 
            // else do nothing

//////// EITHER IN TURN OR WAITING FOR END AND CHANGE IN DIRECTION //////////
        } else if ((extract_sign(current_angle) != extract_sign(next_angle))) {
...
/////// IF ALREADY IN A TURN //////////
        } else if (current_state == IN_TURN) {
            // add current node to turn
            turn.intensities[current_angle_cat] += 1;
...
///////// IF WAITING FOR THE END OF A TURN ///////////
        } else if (current_state == WAIT_END_TURN) {
            // add current node to turn
            turn.intensities[current_angle_cat] += 1;
...
// remove outliers
    for (int i = 0; i < max_nodes; ++i) {
        ...
        // if current angle is of opposite sign compared to both previous and next angle, change sign
        if ((extract_sign(angles[previous_node]) != extract_sign(angles[i])) & (extract_sign(angles[i]) != extract_sign(angles[next_node]))) {
            angles[i] = (-1) * angles[i];
        }
    }
...
void Kart::update(int ticks)
{
    ...
    if (m_controller->isLocalPlayerController()) {
        updatePositionAdIfDifferent(ticks);
    ...
    if (world_player) {
        ...
        if (sector_player) {
            ...
            if (tick_counter >= ticks_to_wait){
                ...
                int intensity = calculateIntensity(relativeDistance);
                char direction = calculateDirection(relativeDistance);
...
char Kart::calculateDirection(float value) {
    if (value < 0) {
        return 'l';
    } else if (value > 0) {
        return 'r';
...
SFXBase* Kart::getNextEmitter()
{
    ...
    m_emitters[m_emitter_id]->setVolume(0.5f);
